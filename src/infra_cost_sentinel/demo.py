...
from .models import CostSummary, ServiceCost, DailyCost, ResourceCost, ResourceStatus
...
class DemoDataGenerator:
    ...
    def generate_cost_summary(self, start_date: date | None = None, end_date: date | None = None) -> CostSummary:
        ...
        daily_base = self.monthly_budget / 30
        ...
        for service_name, service_code, weight in self.SERVICES:
            ...
            by_service.append(ServiceCost(
                service=service_name, service_code=service_code, amount=actual_amount,
                percentage=0, daily_costs=daily_costs,
            ))
        ...
        return CostSummary(
            ...
            by_service=by_service, by_region=by_region, previous_period_cost=previous_cost,
            cost_change_percent=cost_change, projected_monthly_cost=projected_monthly,
            projected_annual_cost=projected_annual,
        )
    
    def generate_resources(self, num_ec2: int = 8, num_rds: int = 3, num_ebs: int = 12, num_eip: int = 5) -> list[ResourceCost]:
    ...
    def _generate_ec2_instances(self, count: int) -> list[ResourceCost]:
        ...
        for i in range(count):
            ...
            instances.append(ResourceCost(
                resource_id=f"i-{random.randint(10000000, 99999999):08x}",
                resource_type="EC2::Instance", resource_name=name, region=region,
                account_id="123456789012", monthly_cost=monthly_cost, daily_cost=daily_cost,
                status=status, tags={"Name": name, "Environment": random.choice(["prod", "dev", "staging"])},
                potential_savings=savings, recommendation=rec,
            ))
        return instances
    ...
    def _generate_rds_instances(self, count: int) -> list[ResourceCost]:
        ...
        for i in range(count):
            ...
            instances.append(ResourceCost(
                resource_id=name, resource_type="RDS::DBInstance", resource_name=name,
                region=region, account_id="123456789012", monthly_cost=monthly_cost,
                daily_cost=daily_cost, status=status, potential_savings=savings, recommendation=rec,
            ))
        return instances
    ...
    def _generate_ebs_volumes(self, count: int) -> list[ResourceCost]:
        ...
        for i in range(count):
            ...
            volumes.append(ResourceCost(
                resource_id=f"vol-{random.randint(10000000, 99999999):08x}",
                resource_type="EC2::Volume", resource_name=name, region=region,
                account_id="123456789012", monthly_cost=monthly_cost, daily_cost=daily_cost,
                status=status, tags={"Name": name}, potential_savings=savings, recommendation=rec,
            ))
        return volumes
    ...
    def _generate_elastic_ips(self, count: int) -> list[ResourceCost]:
        ...
        for i in range(count):
            ...
            eips.append(ResourceCost(
                resource_id=f"eipalloc-{random.randint(10000000, 99999999):08x}",
                resource_type="EC2::EIP", resource_name=ip, region=region,
                account_id="123456789012", monthly_cost=monthly_cost, daily_cost=daily_cost,
                status=status, potential_savings=savings, recommendation=rec,
            ))
        return eips
    
    def _estimate_ec2_hourly_cost(self, instance_type: str) -> Decimal:
        costs = {"t3.micro": "0.0104", "t3.small": "0.0208", "t3.medium": "0.0416", "t3.large": "0.0832", "m5.large": "0.096", "m5.xlarge": "0.192", "c5.large": "0.085"}
