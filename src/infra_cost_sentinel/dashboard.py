...
from .analyzer import AnalysisReport, TrendDirection, CostCategory
...
class CostDashboard:
    ...
    def display_service_breakdown(self, summary: CostSummary, top_n: int = 10) -> None:
        ...
        for service in summary.by_service[:top_n]:
            ...
            table.add_row(
                service.service[:40], f"${service.amount:.2f}",
                pct_bar, trend,
            )
    ...
    def display_resources(self, resources: list[ResourceCost], show_all: bool = False) -> None:
        ...
        for resource in display_resources[:20]:
            ...
            rec = resource.recommendation[:30] + "..." if resource.recommendation and len(resource.recommendation) > 30 else (resource.recommendation or "")
            table.add_row(
                f"{status_icon} {resource.resource_id[:20]}", resource.resource_type,
                resource.region, f"${resource.monthly_cost:.2f}",
                f"${resource.potential_savings:.2f}", rec,
            )
        self.console.print(table)
    ...
    def display_analysis_report(self, report: AnalysisReport) -> None:
        ...
        self.console.print()
        efficiency_style = "green" if report.efficiency_score >= 80 else ("yellow" if report.efficiency_score >= 60 else "red")
    ...
    def _display_alerts(self, alerts: list) -> None:
        ...
        for alert in alerts:
            ...
            table.add_row(f"[{severity_style}]{alert.severity.upper()}[/{severity_style}]", alert.title, alert.message[:50])
        self.console.print(table)
    ...
    def _display_savings(self, opportunities: list, total: Decimal) -> None:
        table = Table(title=f"Savings Opportunities (Total: ${total:.2f}/month)", box=box.ROUNDED, header_style="bold green")
        ...
        for opp in sorted(opportunities, key=lambda x: x.potential_savings, reverse=True)[:10]:
            ...
            table.add_row(opp.category, opp.description[:40], f"${opp.potential_savings:.2f}", f"[{effort_style}]{opp.effort}[/{effort_style}]")
        self.console.print(table)
