...
class AWSFetcher:
    ...
    def fetch_cost_summary(self, start_date: date | None = None, end_date: date | None = None, granularity: str = "DAILY") -> CostSummary:
        if end_date is None:
            end_date = date.today()
        ...
        for service_name, amount in sorted(service_totals.items(), key=lambda x: x[1], reverse=True):
            if amount > Decimal("0.01"):
                ...
                by_service.append(ServiceCost(
                    service=service_name, service_code=service_name.replace(" ", "").lower(),
                    amount=amount, percentage=percentage, daily_costs=service_daily.get(service_name, []),
                ))
                ...
                for reservation in response.get("Reservations", []):
                    for instance in reservation.get("Instances", []):
                        ...
                        resources.append(ResourceCost(
                            ...
                            resource_name=name, region=region, account_id=account_id,
                            monthly_cost=monthly_cost, daily_cost=daily_cost, status=status,
                            tags=tags, potential_savings=savings, recommendation=recommendation,
                        ))
            except ClientError:
                continue
        return resources
        ...
        for region in self.regions:
            try:
                ...
                for db in response.get("DBInstances", []):
                    ...
                    resources.append(ResourceCost(
                        resource_id=db_id, resource_type="RDS::DBInstance",
                        resource_name=db_id, region=region, account_id=account_id,
                        monthly_cost=monthly_cost, daily_cost=daily_cost, status=resource_status,
                        potential_savings=savings, recommendation=recommendation,
                    ))
            except ClientError:
                continue
        return resources
        ...
        for region in self.regions:
            try:
                ...
                for volume in response.get("Volumes", []):
                    ...
                    resources.append(ResourceCost(
                        resource_id=volume_id, resource_type="EC2::Volume",
                        resource_name=name, region=region, account_id=account_id,
                        monthly_cost=monthly_cost, daily_cost=daily_cost, status=resource_status,
                        tags=tags, potential_savings=savings, recommendation=recommendation,
                    ))
        ...
        for region in self.regions:
            try:
                ...
                for address in response.get("Addresses", []):
                    ...
                    resources.append(ResourceCost(
                        resource_id=allocation_id or public_ip, resource_type="EC2::EIP",
                        resource_name=public_ip, region=region, account_id=account_id,
                        monthly_cost=monthly_cost, daily_cost=daily_cost, status=resource_status,
                        potential_savings=savings, recommendation=recommendation,
                    ))
            except ClientError:
    ...
    def _estimate_rds_hourly_cost(self, instance_class: str) -> Decimal:
        cost_map = {
            ...
            "db.m5.large": "0.171", "db.m5.xlarge": "0.342", "db.r5.large": "0.240",
        }
        return Decimal(cost_map.get(instance_class, "0.20"))
    
    def _estimate_ebs_monthly_cost(self, volume_type: str, size_gb: int) -> Decimal:
        price_map = {"gp2": "0.10", "gp3": "0.08", "io1": "0.125", "io2": "0.125", "st1": "0.045", "sc1": "0.025", "standard": "0.05"}
